...
const SCHEME_OPTIONS = [
  { value: 'H-04', label: 'Mahaweli H-04 (Thalawa)', center: [8.3346, 80.5054] },
  { value: 'H-05', label: 'Mahaweli H-05 (Galnewa)', center: [8.1996, 80.3777] },
  { value: 'H-07', label: 'Mahaweli H-07 (Nochchiyagama)', center: [8.2671, 80.2209] },
];
...
const FarmerOnboarding = () => {
  ...
  const [mapLayer, setMapLayer] = React.useState<'satellite' | 'terrain'>('satellite');
  const [locationScreenshot, setLocationScreenshot] = React.useState<File | null>(null);
  const [locationScreenshotPreview, setLocationScreenshotPreview] = React.useState<string | null>(null);
  ...
  const handleSchemeChange = (nextSchemeId: string) => {
    ...
    const selected = SCHEME_OPTIONS.find((scheme) => scheme.value === resolvedSchemeId);
    if (!selected) {
      return;
    }
  ...
  const handleMapLocationPick = React.useCallback((lat: number, lng: number) => {
  ...
  const handleUseCurrentLocation = () => {
    if (typeof navigator === 'undefined' || !navigator.geolocation) {
      setMessage({ type: 'error', text: 'Geolocation is not available on this browser.' });
      return;
    }
    setLocating(true);
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setMessage({ type: 'error', text: 'Unable to fetch current location. Please select on the map.' });
        setLocating(false);
      },
  ...
  const handleLocationScreenshotChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) {
      return;
    }
    if (!file.type.startsWith('image/')) {
      setMessage({ type: 'error', text: 'Please upload an image file (PNG, JPG, WEBP).' });
      return;
    }
    if (file.size > 6 * 1024 * 1024) {
      setMessage({ type: 'error', text: 'Screenshot must be smaller than 6MB.' });
      return;
    }
  ...
  const handleFieldCreate = async () => {
    ...
    if (!canCreateField) {
      setMessage({ type: 'error', text: 'Please enter a field name and a valid area.' });
      return;
    }
    if (!isValidArea) {
        ...
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          ...
          {createdFieldId ? (
            <div className="card" style={{ padding: 48, textAlign: 'center' }}>
              ...
              <h2 style={{ fontSize: 24, marginTop: 16 }}>Field created, {displayName}</h2>
              <p className="muted" style={{ fontSize: 14, marginTop: 8 }}>
                This field is now registered to this farmer account. You can view it from the field workspace or add another field.
              </p>
              <div style={{ display: 'flex', gap: 10, marginTop: 24, justifyContent: 'center', flexWrap: 'wrap' }}>
                <a href={`/farmer/field/${createdFieldId}`} className="btn btn-primary">View field →</a>
                <a href="/farmer/fields" className="btn btn-ghost">My fields</a>
                <button type="button" className="btn btn-ghost" onClick={resetForm}>Add another field</button>
              </div>
            </div>
          ) : (
            <div className="card" style={{ padding: 28 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', letterSpacing: '0.08em' }}>NEW FIELD</div>
              <h2 style={{ fontSize: 22, marginTop: 4 }}>Enter your field details</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14, marginTop: 22 }}>
                ...
                <div className="field">
                  ...
                  <select className="select" value={schemeId} onChange={(e) => handleSchemeChange(e.target.value)} disabled={submitting}>
                ...
                <div className="field" style={{ gridColumn: '1 / -1' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                    <button type="button" className="btn btn-ghost btn-sm" onClick={handleUseCurrentLocation} disabled={submitting || locating}>
                ...
                <div className="field" style={{ gridColumn: '1 / -1' }}>
                  ...
                  <div style={{ border: '1px dashed var(--border)', borderRadius: 10, background: 'var(--surface)', padding: 12 }}>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                      <label className="btn btn-ghost btn-sm" style={{ cursor: submitting ? 'not-allowed' : 'pointer' }}>
                        Choose image
                        <input
                          type="file"
                      ...
                      {locationScreenshot && (
                        <button type="button" className="btn btn-ghost btn-sm" onClick={clearLocationScreenshot} disabled={submitting}>
                          Remove image
                        </button>
                      )}
                      <span className="tiny muted">
                        {locationScreenshot ? `Attached: ${locationScreenshot.name}` : 'Add a screenshot to document plot boundaries or landmarks.'}
                      </span>
                    </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 24, justifyContent: 'space-between' }}>
                <a href="/farmer/fields" className="btn btn-ghost" aria-disabled={submitting}>
                  ← Back
                </a>
                <button className="btn btn-primary" onClick={handleFieldCreate} disabled={!canCreateField || submitting}>
